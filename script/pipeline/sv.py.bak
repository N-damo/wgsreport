    ...
    def dellySV(self):
        ...
        if len(self.groups) >1:
            for sample in self.groups:
                ...
                bams.append(bam)
            step1='{tool} call -x {excludeTemplates} -o sv.bcf -g {reference} {bams} ;'.format(tool=self.delly,reference=self.reference,sample=' '.join(bams),excludeTemplates=self.excludeTemplates)
