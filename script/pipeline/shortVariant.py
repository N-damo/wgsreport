...
class ShortVariant(object):
    ...
    beagle = '/share/data3/lianlin/wgsreport/script/pipeline/beagle.py'
    postimp = '/share/data3/lianlin/wgsreport/script/pipeline/postimp.py'
    gatherpostname = '/share/data3/lianlin/wgsreport/script/pipeline/gatherpostname.bat'
    ...
    def trim(self):
        for info_list in self.info_list_by_sample():
            ...
            for parallel_sample in info_list:
                ...
                fastp = 'fastp -j {sample}.json -h {sample}.html -w 4 -i {fq1} -I {fq2} -o {library}.{sample}_r1.fq.gz -O {library}.{sample}_r2.fq.gz ;'.format(
                    fq1=fq1, fq2=fq2, library=library, sample=sample)
                part.append(fastp)
            part.append('wait ;')
    ...
    def MarkDup(self):
        for part, info_list in self.bwa():
            ...
            index = 'samtools index -@ 4 {sample}.dedup.bam ;'.format(
                sample=sample)
            part.append(index)
            CollectInsertSizeMetrics = '{tool} CollectInsertSizeMetrics -I {sample}.dedup.bam -O {sample}.insert_size_metrics.txt -H {sample}.insert_size.pdf ;'.format(
                tool=self.gatk, sample=sample)
            part.append(CollectInsertSizeMetrics)
            CollectWgsMetrics = '{tool} CollectWgsMetrics -I {sample}.dedup.bam -O {sample}.wgs_metrics.txt -R {reference} ;'.format(
                tool=self.gatk, sample=sample, reference=self.reference)
            part.append(CollectWgsMetrics)
            bedgraph = "samtools bedcov {bed} {sample}.dedup.bam|awk '{{print $1,$2,$3,$4/($3-$2)}}'|tr ' ' '\\t' >{sample}_bedgraph.bed ;".format(
                bed=self.makewindow, sample=sample)
    ...
    def genotype(self):
        ...
        annovar = "perl {tool} multiplex.vcf.gz {humandb} -buildver hg38 -out annovar -otherinfo -remove -protocol refGene,cytoBand,exac03,gnomad_exome,gnomad_genome,clinvar_20170905 -operation g,r,f,f,f,f --vcfinput -nastring . -polish -thread 5 ;".format(
            tool=self.annovar, humandb=self.humandb)
        allsplit = "cd {workspace}; python3 {beagle} ../multiplex.vcf.gz ; ".format(
            workspace=os.path.join(self.working_space, 'workspace'), beagle=self.beagle)  # 生成multiplex1.vcf.gz，构建索引，生成allsplit.bat,.pbs
        sge = "#$ -N {batch}\n#$ -pe smp 32\n#$ -q all.q\n#$ -cwd\nset -e\ncd {working_space}\nsource ~/.bash_profile\n".format(
        ...
        part = [BaseRecalibrator, ApplyBQSR, flagstatx,
                covstat, cram, UnifiedGenotyper, annovar, allsplit]
    ...
    def run_qsub_step1_step2(self):
        ...
        while 1:
            ...
            if len(_) == 0:
                ...
                logging.debug('echo begin genotyping')
                break
            else:
                time.sleep(600)
        return step2_ID
    ...
    def run_pbs_step3(self, step2_ID):
        ...
        while 1:
            ...
            if step2_ID not in qstat_ID:
                logging.debug('echo end genotyping')
                files = os.listdir(workspace)
                pbs = [i for i in files if i.endswith('.pbs')]
                pbs_job_ID = []
                for sample in pbs:
                    cmd = 'qsub {sample} ;'.format(
                        sample=os.path.join(workspace, sample))
                    logging.debug(cmd)
                    p = subprocess.Popen(
                        cmd, shell=True, stdout=subprocess.PIPE)
                    ID = p.communicate()[0].split()[2]
                    pbs_job_ID.append(ID)
                logging.debug('echo begin imputation')
                break
            else:
                time.sleep(600)
        return pbs_job_ID
