...
class SampleQC(object):
    columns = ['Samples', 'InsertSize_mean(bp)', 'Insert_std(bp)', 'Mapped_rate(%)', 'PCR_duplication(%)',
               'MEAN_COVERAGE(X)', 'MEDIAN_COVERAGE(X)', 'Coverage_at_least_1X(%)', 'Coverage_at_least_5X(%)']
    ...
    def __init__(self):
        if os.path.exists('1.qc/after_filtering.csv') and os.path.exists('2.mapping/mapping_stat.csv'):
            ...
            raise OSError(
                'no 1.qc/after_filtering.csv or 2.mapping/mapping_stat.csv')
