...
class MarkDuplicate(CollectInsertSizeMetrics):
    ...
    def dedup_log(self):
        ...
        for sample in self.collect_file:
            ...
            UNPAIRED_READS_EXAMINED, READ_PAIRS_EXAMINED, UNMAPPED_READS,PERCENT_DUPLICATION,ESTIMATED_LIBRARY_SIZE = rec[1], rec[2], rec[4],rec[-2],rec[-1]
            #sample[sample]['Unmerged_reads'] = UNPAIRED_READS_EXAMINED
            #sample[sample]['Merged_reads'] = READ_PAIRS_EXAMINED
            dedup[sample]['Mapped_rate(%)'] = round((int(ESTIMATED_LIBRARY_SIZE)-int(UNMAPPED_READS))/int(ESTIMATED_LIBRARY_SIZE),4)
