...
class CollectWgsMetrics(CollectInsertSizeMetrics):
    ...
    def parse_metric(self):
        ...
        for sample in self.collect_file:
            ...
            MEAN_COVERAGE,MEDIAN_COVERAGE,PCT_1X, PCT_5X, PCT_10X, PCT_20X, PCT_30X = rec[1],rec[3],rec[12],rec[13],rec[14],rec[16],rec[18]
            ...
            histogram.to_csv('{module}/{sample}/depth_distribution.csv'.format(module=self.module,sample=sample),index=False)
    ...
    def plot_his(self,histogram,sample):
        ...
        plt.clf()
        plt.plot('coverage','count',data=histogram,color='mediumvioletred',linestyle='solid')
        plt.title('depth_distribution')
        plt.xlabel('depth')
        plt.ylabel('percent of reads(%)')
        plt.savefig('{module}/{sample}/depth_distribution.png'.format(module=self.module,sample=sample),format='png',dpi=200)
        plt.close()
    ...
    def plot_cdf(self,histogram,sample):
        ...
        histogram['cumulative']=histogram['cumulative']/sum(histogram['count'])
        plt.clf()
        plt.plot('coverage','cumulative',data=histogram,color='blue',linestyle='solid')
        plt.title('depth_cumulative.png')
        plt.xlabel('coverage')
        plt.ylabel('acumulative reads proportion(%)')
        plt.savefig('{module}/{sample}/depth_cumulative.png'.format(module=self.module,sample=sample),format='png')
        plt.close()
        histogram=histogram[['coverage','cumulative']]
        histogram.to_csv('{module}/{sample}/depth_cumulative.csv'.format(module=self.module,sample=sample),index=False,header=True)


    def genome_cov(self,sample):
        df=pd.read_csv('{}_bedgraph.bed'.format(sample),sep=' ',header=None)
        ...
        for chr in df['chr'].drop_duplicates():
            ...
            data=df[df['chr']==chr]
            plt.plot(range(len(data)),data['depth'],label=chr, marker='', color='green', linewidth=1.9, alpha=0.9)
            plt.axis(ymin=0,xmin=0)
            plt.fill_between(range(len(data)),data['depth'], data = df[df['chr']==chr], color='green')
            plt.title(chr, loc='left', fontsize=12, fontweight=0, color='black' )
        #plt.tight_layout()    
        plt.suptitle("the Distribution of Genome Coverage", fontsize=13, fontweight=0, color='black', style='italic', y=1.02)
        fig.text(0.5, 0, 'Chromosome Postion(100kb)', ha='center', va='center')
        fig.text(0, 0.5, 'Average Depth_log2', ha='center', va='center', rotation='vertical')
        plt.savefig('{module}/{sample}/genome_cov.png'.format(module=self.module,sample=sample),format='png',bbox_inches='tight',dpi=200)
        plt.close()
