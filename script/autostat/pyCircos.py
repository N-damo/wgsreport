...
class Circos():
            ...
            if self.snp_or_indel(ref,alt) == self.variant_type:
                ...
                for sampleRecorde in rec.samples.values():
                    ...
                    if GT == (0,0) or GT == (None,None):
                        ...
                        sample_dict[sampleRecorde.name][self.chrom].append(self.position)
        return sample_dict
    ...
    def save_dict(self,all_dict):
        ...
        for sample in all_dict:
            ...
            df1=pd.DataFrame({'chrom':['chr1' for i in range(len(partial['chr1']))],'end':partial['chr1']})
            for chrom in chr_list[1:]:
                length=len(partial[chrom])
                df2=pd.DataFrame({'chrom':[chrom for i in range(length)],'end':partial[chrom]})
            ...
            if sample != 'all':
                df1.to_csv('{module}/{sample}/{variant}_pos.bed'.format(module=self.dir,sample=sample,variant=self.variant_type),index=False,header=False,sep='\t')
            else:
                df1.to_csv('{module}/all_{variant}_pos.bed'.format(module=self.dir,variant=self.variant_type),index=False,header=False,sep='\t')
            self.bed_intersect(sample)

    def bed_intersect(self,sample):
        if sample == 'all':
            bed_root='{module}/all_{variant}_pos.bed'.format(module=self.dir,variant=self.variant_type)
        else:
            bed_root='{module}/{sample}/{variant}_pos.bed'.format(module=self.dir,sample=sample,variant=self.variant_type)
        bed=BedTool(bed_root)
        window=BedTool(self.window)
        intersect=window.intersect(bed,c=True)#calculate the number of snp/indel in 200kb window
        ...
        cmd='circos -conf circos2.conf -outputdir {outputdir} -outputfile {sample}_{variant}.circos.png'.format(outputdir=outputdir,sample=sample,variant=self.variant_type)
    ...
    def circos(self):
        ...
        if os.path.exists('ticks.conf'):
            ...
            subprocess.call('cp {}/ticks.conf ticks.conf'.format(self.conf),shell=True)
            subprocess.call('cp {}/ideogram.conf ideogram.conf'.format(self.conf),shell=True)
    ...
    def plot_conf(self,sample):
        conf=os.path.join(self.conf,'plots{}_histogram.conf'.format(self.variant_type))
        line=self.pattern_search(conf,'file')
        if sample == 'all':
            conf_edit='file = {module}/all_{m}_pos.bed'.format(m=self.variant_type,module=self.dir)
        else:
            conf_edit='file = {module}/{sample}/{m}_pos.bed'.format(m=self.variant_type,module=self.dir,sample=sample)
        self.edit_conf(conf,'plots{}_histogram.conf'.format(self.variant_type),line, conf_edit)

        snp_line=self.pattern_search('circos.conf','<<include plotsSNP_histogram.conf>>')
        indel_line=self.pattern_search('circos.conf','<<include plotsINDEL_histogram.conf>>')
    ...
    def heatmap_plot(self,bed):
        ...
        plt.savefig(os.path.join(dirname,'{}_marker_density.png'.format(self.variant_type)),format='png',bbox_inches='tight')
        plt.close()
