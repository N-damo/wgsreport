...
class CollectInsertSizeMetrics(object):
    def __init__(self,module):
        self.module=module
        self.collect_file=Search('CollectInsertSizeMetrics').match()#+Search('MarkDuplicate').match()+Search('CollectWgsMetrics').match()
        self.create_log()
        #self.insert_size()
    ...
    def insert_size(self):
        ...
        for sample in self.collect_file:
            ...
            histogram.to_csv('{module}/{sample}/insert_size.csv'.format(module=self.module,sample=sample),index=False,header=True)
    ...
    def plot_insert_size(self,file,sample):
        plt.clf()
        plt.plot('insert_size','count',data=file,color='black',linestyle='solid')
        plt.title('insert size distribution')
        plt.xlabel('insert size')
        plt.ylabel('read percent(%)')
        plt.tight_layout()
        plt.savefig('{module}/{sample}/insert_size.png'.format(module=self.module,sample=sample),format='png',dpi=200)
        plt.close()
