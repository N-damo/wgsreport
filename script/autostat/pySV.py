...
class SV(object):
                ...
                for sampleRecord in rec.samples.values():
                    ...
                    if GT == (0,0) or GT == (None,None):
                        ...
                        if svtype == 'DEL':
                            ...
                            sample_dict[sampleRecord.name]['DEL_length'].append(sv_length)
                        if svtype == 'DUP':
                            sample_dict[sampleRecord.name]['DUP'] += 1
                            sample_dict[sampleRecord.name]['DUP_length'].append(sv_length)
                        if svtype == 'INS':
                            sample_dict[sampleRecord.name]['INS'] += 1
                            sample_dict[sampleRecord.name]['INS_length'].append(sv_length)
                        if svtype == 'INV':
                            sample_dict[sampleRecord.name]['INV'] += 1
                            sample_dict[sampleRecord.name]['INV_length'].append(sv_length)
                        if svtype == 'BND':
                            sample_dict[sampleRecord.name]['BND'] += 1
                            sample_dict[sampleRecord.name]['BND_length'].append(sv_length)
        vcf.close()
        return sample_dict
    ...
    def sv_format(self):
        ...
        df.to_csv('{module}/sv_stat.csv'.format(module=self.module),header=True,index=False)
    ...
    def sv_plot(self,sample_dict):
        ...
        for sample in sample_dict:
            ...
            for i in d:
                ...
                plt.savefig('{module}/{sample}/sv_length_distribution.png'.format(module=self.module,sample=sample),dpi=200,format='png')
            plt.close()
